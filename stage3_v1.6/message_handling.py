...
#        print(key_t)
    ...
    def msg_handling(self):
        ...
        if re.findall(re.compile(self.pat_werewolf_game_start),API.raw_message):
            self.Wwk_obj.resp_to_data('4',room_number,player_name,API.data['user_id'],'','')
            return
        
        if self.Wwk_obj.check_player_in_game(API.data) != "Not in any group":
            self.Wwk_obj.resp_to_data('5',room_number,player_name,API.user_id,'',API.raw_message)
            return
