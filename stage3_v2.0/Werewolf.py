...
class Werewolf_kill:
            ...
            if player_info[2] == 'Owner':
                if self.Sql_obj.check_avail(room_number) == "Room is full":
                    ...
                    if str(int(num_werewolf) + int(num_prophet) + int(num_hunter) + int(num_villager)) != room_info[1]:
                        ...
                        self.Sql_obj.game_role_assign(room_number,num_werewolf,num_villager,num_hunter,num_prophet)
    ...
    def survivors_update(self,room_number):
        res = self.Sql_obj.get_surviving_players(room_number)
        msg = 'Now surviving:\n'
        for i in res:
            msg += "No." + i +"\n"
        self.send_pri_by_group(room_number,msg)
