...
class Sql:
    ...
    def set_room_status_update(self,room_number,input_key_name,input_content):
        ...
        self.cursor.execute(sql_update)
        self.conn.commit()
        print("Room "+room_number+"\n"+input_key_name+" has been set as "+input_content)
        API.send("Room "+room_number+"\n"+input_key_name+" has been set as "+input_content,"group","206419158")
        self.survivors_update(room_number)
...
#----------------------Nucleic_stat-----------------------
    def get_undone_person_query(self):
        sql_query = 'select * from nucleic_test where have_done_test="False"'
        self.cursor.execute(sql_query)
        res = self.cursor.fetchall()
        return res
    
    def update_undone_person_update(self,id):
        sql_update = 'update nucleic_test set have_done_test="True" where id="%s"'%(id)
        self.cursor.execute(sql_update)
        self.conn.commit()

    def day_end_update(self):
        sql_update = 'update nucleic_test set have_done_test="False"'
        self.cursor.execute(sql_update)
        self.conn.commit()

    def add_name_list(self,id,name):
        sql_insert = 'insert into nucleic_test(id,name,have_done_test) values("%s","%s","False")'%(id,name)
        self.cursor.execute(sql_insert)
        self.conn.commit()

    def person_find_query(self,id):
        sql_query = 'select * from nucleic_test where id="%s" and have_done_test="True"'%(id)
        self.cursor.execute(sql_query)
        res = self.cursor.fetchall()
        if res:
            return True
    ...
    def is_adm_query(self,id):
        sql_query = 'select * from adm where id="%s"'%(id)
        self.cursor.execute(sql_query)
        res = self.cursor.fetchall()
        if res:
            return True
