...
class Sql:
    ...
    def id_assign_update(self,room_number):
        ...
        for i in range(0,len(res)):
            ...
            self.conn.commit()
            API.send("Your id is "+str(i+1),"private",res[i][0])
    ...
    def game_role_assign(self,room_number,num_of_werewolf,num_of_villager,num_of_hunter,num_of_prophet):
        ...
        for i in range(0,num_of_werewolf):
            ...
            player_num -= 1
        API.send_privately_by_group(werewolves_user_id,"You are the werewolf")
        villagers_user_id = []
        ...
        for i in range(0,num_of_prophet):
            ...
            not_assigned_players.pop(j)
            player_num -= 1
        API.send_privately_by_group(prophets_user_id,"You are the hunter")
    ...
    def vote_update(self,id_to_vote,voter_id,room_number):
        ...
        if id_to_vote == '0':
            ...
            if room_info != "No such room":
                if room_info[4] == "False":
                    ...
                    self.send_pri_by_grp(room_number,"Player No."+voter_id+" has abandoned voting")
        ...
        if q_res:
            ...
            self.cursor.execute(sql_update_voter,data_voter)
            self.conn.commit()
            self.send_pri_by_grp(room_number,"No."+voter_id + " has voted No." + id_to_vote)
    ...
    def check_game_role_living_query(self,game_role,room_number):
        ...
        sql_query = 'select * from werewolf where game_role="%s" and room_number="%s"'
        self.cursor.execute(sql_query)
        res = self.cursor.fetchall()
        return len(res)
    ...
    def eliminate_update(self,room_number):
        ...
        if res[0][1] != res[1][1]:#exist a most_voted player
            ...
            self.send_pri_by_grp(room_number,"Player No."+res[0][7]+" is eliminated")
            ...
            API.send_privately_by_group(room_number,"Start voting again,until someone is eliminated,NO abandoning voting")
            self.send_pri_by_grp(room_number,"Start voting again,until someone is eliminated")
    ...
    def killed_by_werewolf_update(self,id_to_act_on,room_number):
        ...
        self.send_pri_by_grp(room_number,"No."+id_to_act_on+" is out")
        return
    ...
    def set_room_status_update(self,room_number,input_key_name,input_content):
        ...
        self.send_pri_by_grp(room_number,input_key_name+" has been set as "+input_content)
        self.survivors_update(room_number)
    ...
    def survivors_update(self,room_number):
        ...
        self.send_pri_by_grp(room_number,msg)
